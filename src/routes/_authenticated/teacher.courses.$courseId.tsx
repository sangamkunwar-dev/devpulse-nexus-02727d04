...
function TeacherCourseManager() {
  ...
  useEffect(() => {
    const channel = supabase.channel(`course-manager-${courseId}`).on("postgres_changes", { event: "*", schema: "public", table: "course_enrollments", filter: `course_id=eq.${courseId}` }, () => void load()).subscribe();
