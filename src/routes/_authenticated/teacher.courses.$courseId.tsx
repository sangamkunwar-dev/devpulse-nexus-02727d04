...
type Earning = { amount: number; currency: string };
...
function TeacherCourseManager() {
  ...
  const load = async () => {
    const [{ data: courseData }, { data: lessonData }, { data: enrollmentData }, { data: earningsData }] = await Promise.all([
      ...
      (supabase as any).from("teacher_earnings").select("amount,currency").eq("course_id", courseId),
    ]);
    setCourse(courseData as Course | null); setLessons((lessonData ?? []) as Lesson[]); setStudents((enrollmentData ?? []) as Enrollment[]); setEarnings((earningsData ?? []) as Earning[]);
  };
  useEffect(() => { void load(); }, [courseId]);
  useEffect(() => {
    const channel = supabase.channel(`course-manager-${courseId}`).on("postgres_changes", { event: "*", schema: "public", table: "course_enrollments", filter: `course_id=eq.${courseId}` }, () => void load()).on("postgres_changes", { event: "*", schema: "public", table: "teacher_earnings", filter: `course_id=eq.${courseId}` }, () => void load()).subscribe();
